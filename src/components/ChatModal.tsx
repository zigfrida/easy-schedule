...
function ChatModal({ appointmentId, secondaryUser, title, visible, onClose }: Props) {
    ...
    const chatDao = useMemo(
        () => createFirebaseDao<AppointmentMessage>('appointment', appointmentId, 'messages'),
        [appointmentId],
    );

    useEffect(() => {
        setLoading(true);
        const unsubscribe = chatDao.subscribe((data) => {
            setMessages(data);
            setLoading(false);
        });
        return unsubscribe;
    ...
    return (
        ...
        >
            ...
            <DialogContent dividers style={{ backgroundColor: 'gainsboro' }}>
                <DialogContentText
